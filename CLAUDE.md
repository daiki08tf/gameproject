...
Continue UIX-7 — Motion, Feedback and Accessibility Pass (`UI_OVERHAUL_ROADMAP.md` §6) with **Phase 2 — screen-level transition rules** next, followed by Phase 3 (a real damage/loot/unlock/discovery feedback system, plus the systemic px→rem/`clamp()` dynamic-text retrofit Phase 1's audit named but deliberately deferred). Phase 1 already closed reduced-motion support, safe-area handling, the no-animation-blocks-farming check (all three audited-and-already-satisfied), the highest-leverage readable-dynamic-text-sizing fix (the viewport meta tag was disabling pinch-zoom app-wide — see `UI_OVERHAUL_ROADMAP.md` §6 UIX-7 Phase 1 for the full record), a numeric color/contrast audit, and a keyboard/focus helper (`js/patches/overlayA11y.js`) applied to all three ad-hoc overlays in the app. This remains a different shape of phase than UIX-6's per-screen emoji/token migration — read PROJECT_GUIDE.md and RELEASE_CANDIDATE_AUDIT.md per the read-before-changing-code list before each new slice, keep each phase's diff scoped to one coherent concern (not one screen family), add regression tests, and do a live-viewport pass before marking a phase complete. Screen-transition work in particular touches the `showScreen()` pattern duplicated across a dozen+ files and CSS's inability to animate across a `display:none` boundary without JS-coordinated two-step class toggling — audit every call site carefully before changing this shared behavior, per the DOM-safety rules above. Also keep the found-and-fix habit UIX-6/UIX-7 Phase 1 both relied on: a static scan or a prior batch's claim of "complete" doesn't preclude a live-viewport pass surfacing a real gap (Phase 1 found two more emoji leaks this way, in `abyssRunUi.js` and `battleIntegration3.js`/`combat2Elements.js`) — fix what's found, don't defer it just because it's outside the current slice's original file list. `UIX0_SOURCE_AND_OWNERSHIP_AUDIT.md` §10–§17 records the closed UIX-6 program for reference on established conventions (Dark Chronicle tokens, the emoji-free contract, the DOM-safety/MutationObserver idempotency rules) that UIX-7 must continue to respect.
