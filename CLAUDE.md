...
- **Observed Branches M6 → M9 → M7 + Arc VI** (`OBSERVED_BRANCHES_MULTIVERSE_ROADMAP.md`, `STORY_CANON.md` "Current development direction"): approved new content direction, not yet started. Next concrete step is M6 — 深緑消失域, the second Branch of the same Ch2 forest region, whose design (divergence point, technology profile, gear themes, boss 根無き森核・NULL CANOPY) is already authored in the roadmap doc. After M6: M9's confirmed first pick (灼熱の火山), then M7's comparative Codex/Chronicle record feeding directly into a new Ch36 opening Arc VI. Read `OBSERVED_BRANCHES_MULTIVERSE_ROADMAP.md`'s Decision log in full before starting — it also records why M5 (broader gear expansion) was deliberately reordered to follow M6 rather than precede it. For Story/canon-facing text, also read `STORY_CANON.md` and `WORLD_LORE_BIBLE.md` per the read-before-changing-code list; the Arc VI beat depends on Open Mysteries #8/#9 in `WORLD_LORE_BIBLE.md` and must not resolve #1/#2.
