...
UIX-0 through UIX-5 are complete: source implementation and the live-viewport acceptance gate (390×844/375×667/desktop, fresh-save and progressed-save) both. UIX-6 batch 1 (Status/Job/Rebirth) and batch 2 (Companion/Monster Ranch) are also complete, same gate. See `UIX0_SOURCE_AND_OWNERSHIP_AUDIT.md` §10–§14 for the full record. UIX-6 batch 3 (Settlement facilities) is next; `UI_OVERHAUL_ROADMAP.md` §10 names the remaining batch order and the emoji debt already located at the source level.
...
Continue UIX-6 — Character, Ranch, Settlement, Records and Endgame, with batch 3 (Settlement facilities) next (scope, remaining batch order and known emoji debt: `UI_OVERHAUL_ROADMAP.md` §10). Each batch is a separate PR unless the audit proves it genuinely small. Follow the same protocol as UIX-3–6b2: run a full static source scan for ground-truth emoji counts across every file that renders into the target screen's container, not just the files a prior note named — a prior audit/live pass only proves what it already knew or happened to exercise (see `UIX0_SOURCE_AND_OWNERSHIP_AUDIT.md` §13–§14, and note `js/patches/settlementRanch3Ui.js`'s 5 glyphs were deferred into this batch from batch 2 on container-ownership grounds), focused tests, full `npm test`, `npm run test:syntax`, ratchet `scripts/uix-emoji-check.js`'s ceiling down by the exact count removed, and a live-browser pass at 390×844/375×667/desktop before checking a batch complete. Do not claim it complete until that live gate is recorded, matching the standard UIX-0–6b2 set.
