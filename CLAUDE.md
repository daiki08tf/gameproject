...
- **Observed Branches M6 → M9 → M7 + Arc VI → M8/M10/M11/M12 → M9 continuation (Cluster 3): COMPLETE.** M6 (深緑消失域), M5 (Divergent Technology Gear I), M9's Cluster 2 first pick (灼熱の火山・炎帝領), M7 (comparative Codex/Chronicle record), Ch36 (Arc VI opening), M8 (Branch Equipment II familiar variants), M10 (Enemy/Nemesis Branch integration, including the optional Nemesis pilot), M11 (Endgame Branch chase verification), M12 (completion audit), and — as a follow-up continuation after M12's closeout — M9's Cluster 3 first pick (全権域・機界監査層, rooted in Ch28) have all shipped — see `OBSERVED_BRANCHES_M12_AUDIT.md` for the M0–M12 closeout cross-check and `OBSERVED_BRANCHES_MULTIVERSE_ROADMAP.md`'s Decision log for the shipped-order rationale, including the Cluster 3 entry explaining why Ch28 (not the separate Phase 9 "Machine World" Secret Realm dungeon) is that Branch's Prime Region. **Remaining open work**: M9 is still partial — only The Veil cluster remains queued; it can reuse the exact M9→M10 pipeline already built for 灼熱の火山/機界監査層 without further architecture work. For Story/canon-facing text, read `STORY_CANON.md` and `WORLD_LORE_BIBLE.md` per the read-before-changing-code list before continuing M9.
