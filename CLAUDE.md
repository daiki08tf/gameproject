...
Three independent workstreams currently have open next actions:

- **Runtime debug backlog** (`CODEBASE_DEBUG_AUDIT_2026-09-05.md`): DBG-01 (Rift Key entry) merged to main as PR #411 (`a3931a068b9ed32cfe1d3362252450cd629b3a54`) and covered by `tests/rift-entry.test.js`; a live-browser spot check this session (granting a key via `state.addRiftKey`, entering the discovered-branches list, confirming, and starting battle) rendered correctly with no console errors, though the audit's own fuller acceptance walk (390×844/375×667/desktop: Adventure → discovered branches → owned Rift key → confirm/back → confirm/start → battle → retreat/result → select another key; verify key count survives reload and no used-key replay) has not been separately re-run end to end. After that, continue the audit's remaining reachability/save-schema/battle-authority/UI-interaction batches.
- **Observed Branches M0–M12, including M9's full continuation: ALL COMPLETE.** M6 (深緑消失域), M5 (Divergent Technology Gear I), M9's Cluster 2 first pick (灼熱の火山・炎帝領), M7 (comparative Codex/Chronicle record), Ch36 (Arc VI opening), M8 (Branch Equipment II familiar variants), M10 (Enemy/Nemesis Branch integration, including the optional Nemesis pilot), M11 (Endgame Branch chase verification), M12 (completion audit), and — as follow-up continuations after M12's closeout — M9's Cluster 3 (全権域・機界監査層, rooted in Ch28) and Cluster 4 (不断領・月蝕の境界, rooted in Ch19, the last queued cluster) have all shipped — see `OBSERVED_BRANCHES_M12_AUDIT.md` for the M0–M12 closeout cross-check and `OBSERVED_BRANCHES_MULTIVERSE_ROADMAP.md`'s Decision log for the shipped-order rationale, including why Ch28/Ch19 (not the separate Phase 9 "Machine World" Secret Realm dungeon) are those Branches' Prime Regions. **No Observed Branches milestone remains queued.** Any further horizontal content (additional Branch Clusters beyond the roadmap's original preferred list, deeper per-Branch gear) is a new scope decision, not a carried-over item — check with the user before starting more. For Story/canon-facing text, read `STORY_CANON.md` and `WORLD_LORE_BIBLE.md` per the read-before-changing-code list before any such new work.
