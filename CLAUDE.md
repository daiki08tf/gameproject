...
UIX-0 through UIX-5 are complete: source implementation and the live-viewport acceptance gate (390×844/375×667/desktop, fresh-save and progressed-save) both. UIX-6 batch 1 (Status/Job/Rebirth), batch 2 (Companion/Monster Ranch) and batch 3 (Settlement facilities) are also complete, same gate. See `UIX0_SOURCE_AND_OWNERSHIP_AUDIT.md` §10–§15 for the full record. UIX-6 batch 4 (Codex/Rumor/records) is next; `UI_OVERHAUL_ROADMAP.md` §10 names the remaining batch order and the emoji debt already located at the source level, including a Battle/Codex enemy-role-icon finding from batch 3 relevant to this batch.
...
Continue UIX-6 — Character, Ranch, Settlement, Records and Endgame, with batch 4 (Codex/Rumor/records) next (scope, remaining batch order and known emoji debt: `UI_OVERHAUL_ROADMAP.md` §10). Each batch is a separate PR unless the audit proves it genuinely small. Follow the same protocol as UIX-3–6b3: run a full static source scan for ground-truth emoji counts across every file that renders into the target screen's container, not just the files a prior note named — a prior audit/live pass only proves what it already knew or happened to exercise (see `UIX0_SOURCE_AND_OWNERSHIP_AUDIT.md` §13–§15; batch 3 found 58 glyphs across 17 files against a prior count of 33, including two files whose emoji came entirely from data-object `icon` fields with no literal glyphs of their own, and one file discovered only by the live pass), focused tests, full `npm test`, `npm run test:syntax`, ratchet `scripts/uix-emoji-check.js`'s ceiling down by the exact count removed, and a live-browser pass at 390×844/375×667/desktop before checking a batch complete. Also review batch 3's finding that `js/patches/combat3EnemyAI.js` prefixes researched enemies' Battle-screen names with a raw `ENEMY_ROLES` icon (tested, intentional behavior — `tests/codex-enemy-knowledge.test.js`) when scoping this batch's Codex-adjacent work. Do not claim a batch complete until that live gate is recorded, matching the standard UIX-0–6b3 set.
