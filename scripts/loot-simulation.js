...
export function createSeededRandom(seed = 'bladevale-loot') {
  ...
  return function random() {
    // xorshift32: fast, deterministic and sufficient for a Monte Carlo dev tool.
    state ^= state << 13;
    state ^= state >>> 17;
    state ^= state << 5;
...
export function simulateLootCheckpoint({
  ...
  const random = createSeededRandom(`${seed}:${ip}:${itemRarity}:${source}:${count}`);
  ...
  for (let sample = 0; sample < n; sample += 1) {
    ...
    // Practical high-end target: build-defining Legendary + strong affix quality + >=2 Greater.
    if (hasLegendaryEffect && hasMythicPlus && greater >= 2) godRolls += 1;
    // Extreme jackpot: 3 Greater, at least one Ancient affix, and a Legendary Effect.
    if (hasLegendaryEffect && hasAncient && greater >= 3) jackpotRolls += 1;
  }
...
export function formatLootSimulation(results) {
  ...
  lines.push('');
  lines.push('IP | Mythic+ item | Ancient item | G×2 | G×3 | Leg.Effect | Curse | God Roll | Jackpot');
  lines.push('---|---:|---:|---:|---:|---:|---:|---:|---:');
