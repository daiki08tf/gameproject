...
export function initRebirthTabs() {
  ...
  document.querySelectorAll('#rebirthScreen .tab-btn').forEach((btn) => btn.addEventListener('click', () => {
    Audio_.tap(); activeTab = btn.dataset.rtab; inheritArmed = false; selectedArtifactSlot = null; renderRebirth();
  }));
}
...
function renderInheritance(content) {
  ...
  allocation.innerHTML = `<div class="section-heading">継承BP — 基礎能力への投資</div><div class="forge-card"><div class="forge-card-top"><div class="forge-card-name">未使用BP</div><strong>${unspent.toLocaleString()} pt</strong></div><button class="btn-sub" id="resetInheritanceBtn" style="width:100%;margin-top:8px;">振り直す</button></div>${STATS.map((key) => `<div class="forge-card" data-inherit-stat="${key}"><div class="forge-card-top"><div class="forge-card-name">${LABEL[key]}</div><strong>+${Number(state.data.inheritanceAllocated?.[key]||0).toLocaleString()}</strong></div><div class="inheritance-buttons"><button class="btn-sub" data-add="1">+1</button><button class="btn-sub" data-add="10">+10</button><button class="btn-sub" data-add="100">+100</button><button class="btn-sub" data-add="max">MAX</button></div></div>`).join('')}`;
  ...
  allocation.querySelectorAll('[data-inherit-stat]').forEach((row) => row.querySelectorAll('[data-add]').forEach((btn) => btn.addEventListener('click', () => { const available = state.inheritanceUnspentPoints(); const amount = btn.dataset.add === 'max' ? available : Math.min(available, Number(btn.dataset.add)); if (amount > 0 && state.allocateInheritancePoints(row.dataset.inheritStat, amount)) Audio_.pickup(); renderRebirth(); })));
}
...
function renderArtifacts(content) {
  ...
  const head = document.createElement('div'); head.className = 'forge-card';
  head.innerHTML = `<div class="forge-card-top"><div class="forge-card-name">秘宝 / Relic</div><strong>覚醒Rank ${rank}</strong></div><div class="forge-card-sub">Artifactは覚醒で、Relicは覚醒Rank3＋深淵Era到達で順番に解放候補になります。Relicは深淵攻略の節目ごとに新しいビルドルールを追加します。</div><div class="rune-slots" id="artifactSlotsRow"></div>`;
...
function renderArtifactPicker(content, slotIndex) {
  ...
  const currentId = state.data.equippedArtifacts?.[slotIndex]; let html = `<div class="forge-card-top"><div class="forge-card-name">スロット${slotIndex+1}</div></div>`;
  if (currentId) html += `<div class="pick-row"><div>${getArtifact(currentId)?.name || currentId}</div><button data-unset>外す</button></div>`;
  for (const id of state.data.unlockedArtifacts || []) { if (id === currentId) continue; const a = getArtifact(id); if (!a) continue; html += `<div class="pick-row" data-set="${id}"><div><div class="item-name">${a.name}</div><div class="item-stats">${a.desc}</div></div><button>セット</button></div>`; }
