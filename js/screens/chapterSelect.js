...
export function renderChapterSelect(onPick) {
  ...
  if (latestLore) {
    ...
    record.innerHTML = `<div><div class="name">📖 ${latestLore.title}</div><div class="rec">${latestLore.text}</div></div>`;
    list.appendChild(record);
  }

  const realm=state.world2RealmVisibility?.()||{};
  const realmLine=document.createElement('div');
  realmLine.className='stage-card branch';
  const labels=[['mortal','人界'],['heaven','天界'],['underworld','冥界'],['modern','？？？']]
    .filter(([id])=>realm[id]&&realm[id]!=='hidden')
    .map(([id,name])=>`${name}:${realm[id]==='open'?'OPEN':realm[id]==='hint'?'兆候':'???'}`);
  realmLine.innerHTML=`<div><div class="name">WORLD 3.0</div><div class="rec">${labels.join(' / ')||'人界を探索中'}</div></div>`;
  list.appendChild(realmLine);
  ...
  WORLD3_REGIONS.forEach((region)=>{
    ...
    wrap.className='world3-region';
    wrap.style.cssText='margin:10px 0;border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:8px;background:rgba(255,255,255,.025)';
    const header=document.createElement('button');
    header.type='button';
    header.className='btn-sub';
    header.style.cssText='width:100%;text-align:left;display:flex;justify-content:space-between;gap:8px;padding:9px';
    const current=region.chapters.some(n=>{
      const idx=n-1,ch=CHAPTERS[idx];
      return ch&&isChapterUnlocked(idx,(id)=>state.isStageCleared(id))&&!state.isStageCleared(finalStageOf(ch).id);
    });
    ...
    body.className='world3-region-body';
    body.hidden=!current&&region.id!=='frontier';
