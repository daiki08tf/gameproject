...
export const BREAK_RULES=Object.freeze({normal:1,weak:1.6,resist:.65,shatter:.25,bossMult:1.25,brokenDamageMult:1.22,comboBreak:{wildfire:1.20,shock:1.35,necrosis:1.25,shatter:1.40}});
...
export function breakDamage({damage=0,maxHp=1,capacity=20,elementMultiplier=1,boss=false,physical=false,frosted=false,combo=null}={}){const ratio=Math.max(0,damage)/Math.max(1,maxHp);let mult=elementMultiplier>1?BREAK_RULES.weak:elementMultiplier<1?BREAK_RULES.resist:BREAK_RULES.normal;if(boss)mult/=BREAK_RULES.bossMult;let value=Math.max(1,Math.round((1+ratio*capacity*3)*mult));if(physical&&frosted)value+=Math.max(1,Math.round(capacity*BREAK_RULES.shatter));const comboMult=BREAK_RULES.comboBreak?.[combo]||1;return Math.max(1,Math.round(value*comboMult));}
