...
export function bondSkillsFor(companion){const lv=Number(companion?.bondLevel)||Number(bondLevelResolver(companion?.id))||1,out=[];const magical=(companion?.mag||0)>(companion?.atk||0)*1.08;if(lv>=4)out.push(magical?BOND_SKILLS.bond_arcana:BOND_SKILLS.bond_strike);if(lv>=8)out.push(magical?BOND_SKILLS.soul_nova:BOND_SKILLS.soul_fang);return out;}
