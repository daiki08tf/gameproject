...
function buildChapter(ch) {
  ...
  const rewardBase = [{ gold: 30, exp: 20 }, { gold: 45, exp: 32 }, { gold: 60, exp: 46 }, { gold: 90, exp: 70 }];
  const stages = [
    ...
    { id: `${ch.num}-2`, name: `${ch.name}の${STAGE_NAMES[1]}`, recLevel: Math.round(ch.recLevel[0] + (ch.recLevel[1] - ch.recLevel[0]) * 0.25), waves: [{ type: normalId, count: 4, interval: 1.2 }, { type: fastId, count: 3, interval: 1.0 }], rewards: scaleReward(rewardBase[1], mult), dropTable: [{ itemId: `${ch.id}_shield`, weight: 1 }] },
    { id: `${ch.num}-3`, name: `${ch.name}の${STAGE_NAMES[2]}`, recLevel: Math.round(ch.recLevel[0] + (ch.recLevel[1] - ch.recLevel[0]) * 0.5), waves: [{ type: fastId, count: 5, interval: 0.9 }, { type: tankId, count: 2, interval: 2.0 }], rewards: scaleReward(rewardBase[2], mult), dropTable: [{ itemId: `${ch.id}_weapon`, weight: 1 }, { itemId: `${ch.id}_head`, weight: 1 }] },
    { id: `${ch.num}-4`, name: `${ch.name}の${STAGE_NAMES[3]}`, recLevel: Math.round(ch.recLevel[0] + (ch.recLevel[1] - ch.recLevel[0]) * 0.75), waves: [{ type: normalId, count: 5, interval: 1.0 }, { type: fastId, count: 5, interval: 0.8 }, { type: tankId, count: 3, interval: 1.8 }], rewards: scaleReward(rewardBase[3], mult), dropTable: [{ itemId: `${ch.id}_body`, weight: 1 }, { itemId: `${ch.id}_accessory`, weight: 1 }] },
    { id: `${ch.num}-5`, name: `${ch.name}：${ch.enemies.boss}`, recLevel: ch.recLevel[1], boss: true, waves: [{ type: normalId, count: 4, interval: 1.2 }, { type: bossId, count: 1, interval: 0 }], rewards: scaleReward({ gold: 200, exp: 150 }, mult), firstClear: { itemId: `${ch.id}_weapon_epic` }, dropTable: [{ itemId: `${ch.id}_named_${ch.items.named.slot}`, weight: 1 }, { itemId: `rune_effect_${ch.items.named.effect}`, weight: 1 }].concat(ch.items.named2 ? [{ itemId: `${ch.id}_named2_${ch.items.named2.slot}`, weight: 1 }, { itemId: `rune_effect_${ch.items.named2.effect}`, weight: 1 }] : []) },
  ];
  if (ch.branch) stages.push({ id: `${ch.num}-B`, name: `${ch.name}：隠し道（${ch.branch.enemyName}）`, recLevel: Math.round(ch.recLevel[0] + (ch.recLevel[1] - ch.recLevel[0]) * 0.5), branch: true, requires: `${ch.num}-3`, waves: [{ type: normalId, count: 3, interval: 1.2 }, { type: `${ch.id}_branchboss`, count: 1, interval: 0 }], rewards: scaleReward({ gold: 70, exp: 55 }, mult), firstClear: { itemId: `${ch.id}_branch` }, dropTable: [] });
...
for (const ch of CHAPTERS) { const tags = CHAPTER_REGION_TAGS[ch.id] || []; for (const st of ch.stages) st.dropRegionTags = tags; }
...
export function findStage(stageId) {
  if (stageId.startsWith('abyss-')) { const depth = parseInt(stageId.slice('abyss-'.length), 10); if (Number.isFinite(depth) && depth >= 1) return { chapter: null, stage: buildAbyssStage(depth) }; return null; }
