...
export const BOSS_AI_LAYER = {
  ...
  SUMMON_COUNT: 2,
  // バランス再較正：Boss手番のstarvation修正（battleEngine.js）でsummonが
  // 定期的に発動するようになったため、長引いたBoss戦で手下が際限なく
  // 積み上がらないよう同時召喚数の上限を設ける（元指示：6〜8体）
  SUMMON_MAX_ALIVE: 8,
};
...
export const TEXT_BATTLE_LAYER = {
  ...
  //
  // バランス再較正（元指示：Boss大技は無防備で最大HP20〜40%を目安にする）：
  // 実測（_enemyAttackDamage(atk, {mult: BOSS_AI_LAYER.CHARGE_DAMAGE_MULT})を
  // 実際のBoss.atkと標準プレイヤーで直接呼び出した値。本番コードの
  // _resolveBossSpecial()と全く同じ呼び方＝TELEGRAPH_MULT_SCALEは関数内部で
  // 1回だけ適用される）では、旧値=4だと章8で無防備19.0%・章5で無防備
  // 約16%相当と、目安20〜40%の下限にすら届いていなかった（squishy職が
  // 予兆を無視しても即詰みにはならないが、逆に「予兆を見て防御する」意味が
  // 薄いほど軽かった）。ENEMY_SCALING（章別ATK/HP/DEF成長曲線、実時間版
  // からの計算式そのもの）とCHARACTER_LAYER（プレイヤー成長曲線）はどちらも
  // 「原則変更禁止」対象のため一切変更せず、ターン制移行時にのみ導入した
  // このスケール係数だけを4→8へ引き上げて全章一律で予兆の重さを底上げした。
  // 章5・8・10（フル構成プロファイル）で無防備36〜38%、章4（既定プロファイル）
  // で無防備21%と目安20〜40%に収まることを実測済み。章1（既定プロファイル・
  // Boss ATK自体が抑えめ）だけは無防備3%程度に留まるが、これは「易しい
