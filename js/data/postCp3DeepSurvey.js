...
export function deepSurveyByRealmId(realmId){
  const {baseRealmId}=resolveDeepSurveyConditionIds(realmId);
  return CP3_DEEP_SURVEYS.find(x=>x.realmId===baseRealmId)||null;
}
...
function product(conditions,key){
  return conditions.reduce((m,c)=>m*(Number(c?.effect?.[key])||1),1);
}
function sum(conditions,key){
  return conditions.reduce((n,c)=>n+(Number(c?.effect?.[key])||0),0);
}
function conditionedWaves(waves,conditions){
  const countMult=product(conditions,'waveCountMult');
  if(countMult===1)return waves;
  return waves.map(w=>w.deepSurveyApex?{...w}:{...w,count:Math.max(1,Math.round((Number(w.count)||1)*countMult))});
}
...
export function buildDeepSurveyStage(realmId){
  ...
  // V1/V2 accepts one encoded Condition. V3 may intentionally raise this cap.
  const conditions=resolved.conditionIds.map(surveyCondition).filter(c=>c&&c.regionId===def.id).slice(0,1);
  ...
  const conditionEffects=Object.freeze({
    ...
    mpCostMult:product(conditions,'mpCostMult'),
    bossTechniqueIntervalMult:product(conditions,'bossTechniqueIntervalMult'),
    enemyHpMult:product(conditions,'enemyHpMult'),
    enemySpeedMult:product(conditions,'enemySpeedMult'),
    enemyAtkMult:product(conditions,'enemyAtkMult'),
  ...
  return{
    ...base,
    id:encodeDeepSurveyConditionStageId(def.realmId,conditionIds),
    name:`深層観測・${def.realmName.replace('・深層観測','')}${conditionSuffix}`,
    ...
    deepSurveyBaseRealmId:def.realmId,
    deepSurveyConditionIds:conditionIds,
    deepSurveyConditions:conditions,
    deepSurveyConditionEffects:conditionEffects,
