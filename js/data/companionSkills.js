...
export function unlockedCompanionSkills(species,level){return(species?.skills||[]).filter(entry=>level>=(entry.level||1)).map(entry=>getCompanionSkill(entry.id)).filter(Boolean);}
export function chooseCompanionSkill(species,companion,enemies){
  const skills=unlockedCompanionSkills(species,companion.level||1).filter(skill=>(companion.mp||0)>=(skill.mpCost||0));if(!skills.length)return null;
