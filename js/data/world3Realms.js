export const WORLD3_REALM_NODES=Object.freeze([
  ...
  {id:'heaven',icon:'☀️',name:'天界',subtitle:'空の裂け目の向こうにある光の世界',route:'world3-branches'},
  {id:'underworld',icon:'🌑',name:'冥界',subtitle:'深層から死者の声が漏れる世界',route:'world3-branches'},
...
export function world3RealmNodeState(node,visibility={},flags={}){
  ...
  if(raw==='hidden')return{...node,state:'hidden',selectable:false,badge:'',detail:''};
  if(raw==='hint')return{...node,state:'hint',selectable:false,badge:'兆候',detail:node.id==='heaven'?'空の裂け目から光が漏れている。対応する境界鍵があれば門を開けそうだ。':'深い場所ほど死者の声が近い。奈落へ通じる鍵穴がどこかにある。'};
  return{...node,state:'open',selectable:true,badge:'OPEN',detail:node.id==='heaven'?'天門は開いた。発見された分岐から天界へ続く鍵路に挑める。':'奈落門は開いた。発見された分岐から冥界へ続く鍵路に挑める。'};
}
