...
const GROUPS=Object.freeze({martial:new Set(['warrior','fighter','hunter','ninja']),arcane:new Set(['mage','alchemist','scholar','fortune']),support:new Set(['priest','bard','dancer','farmer']),utility:new Set(['thief','merchant','craftsman'])});
...
const ROLE=Object.freeze({
 martial:{title:'武極',ult:[P('critDamageBoost',.20),P('bossDmg',.12),P('dmgBonusAdd',.08)]},arcane:{title:'秘奥',ult:[P('spellDmgAdd',.18),P('cdrAdd',.08),P('mpCostReduce',.07)]},support:{title:'加護',ult:[P('regen',.016),P('bossSpecialMitigation',.15),P('guardMitigation',.08)]},utility:{title:'技巧',ult:[P('executioner',.12),P('dropRateMultAdd',.12),P('dmgBonusAdd',.08)]},spellblade:{title:'魔戦',ult:[P('skillDmgAdd',.13),P('spellDmgAdd',.13),P('bossDmg',.11)]},guardian:{title:'守戦',ult:[P('guardMitigation',.12),P('bossSpecialMitigation',.14),P('bossDmg',.08)]},skirmisher:{title:'遊撃',ult:[P('atkSpeedAdd',.11),P('critDamageBoost',.16),P('executioner',.10)]},mystic:{title:'神秘',ult:[P('spellDmgAdd',.12),P('regen',.010),P('cdrAdd',.06)]},artificer:{title:'錬機',ult:[P('skillDmgAdd',.11),P('spellDmgAdd',.09),P('dmgBonusAdd',.09)]},provisioner:{title:'支度',ult:[P('regen',.009),P('goldMultAdd',.10),P('dropRateMultAdd',.10)]},hybrid:{title:'融星',ult:[P('dmgBonusAdd',.11),P('bossDmg',.10)]}
});
...
function build(job){const[a,b]=job.parents,A=ARCH[a],B=ARCH[b],role=ROLE[roleOf(job)];return Object.freeze([trait(job,A,B),path(job,a,A,'a'),key(job,A,'a',true),path(job,b,B,'b'),key(job,B,'b'),{id:`${job.id}_ultimate`,cost:3,requires:[],requiresAny:[`${job.id}_keystone`,`${job.id}_keystone_b`],kind:'fusionUltimate',name:`奥義・${job.name}極星`,desc:`${role.title}の極致。二系譜のどちらを選んでも到達できる最終星。`,effects:role.ult}]);}
const SIGNATURE_NAMES=Object.freeze({battlemaster:{trait:'羅刹身',a:'武威・修羅道',b:'闘気・連環道',ult:'奥義・阿修羅連環'},spellblade:{trait:'魔装剣',a:'武装魔法',b:'魔導剣理',ult:'奥義・エレメントブレイク'},paladin:{trait:'聖盾',a:'守護騎士道',b:'聖祈騎士道',ult:'奥義・最後の聖域'},sage:{trait:'叡聖回路',a:'秘奥魔導',b:'聖賢祈祷',ult:'奥義・賢者の星界'},assassinfist:{trait:'殺拳',a:'絶命拳路',b:'影殺拳路',ult:'奥義・無影絶殺'}});
