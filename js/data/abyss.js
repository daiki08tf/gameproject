...
export function buildAbyssStage(rawDepth,pactIds=[]){
 const depth=Math.max(1,Math.floor(Number(rawDepth)||1)); const modifiers=modifiersForDepth(depth); const valid=[...new Set(pactIds)].slice(0,maxAbyssPactsForDepth(depth)).filter(id=>abyssPact(id));
