...
export function buildAbyssStage(rawDepth,pactIds=[],options={}){
 ...
 const level=abyssRecommendedLevel(depth),itemPower=abyssTargetItemPower(depth),expBudget=abyssStageExpBudget(depth)*expMult*(bossFloor?1.35:1),goldReward=200*chapterMult(15)*Math.pow(Math.max(1,level/700),.72)*goldMult*(bossFloor?ABYSS_LAYER.BOSS_REWARD_MULT:1),routePrefix=route?`${route.icon} ${route.name} — `:'',stageId=`abyss-${depth}${route?`~${route.id}`:''}`;
