...
/* Session 7 — 探索地点・第二波の秘密Boss。
   fh_overseer: 炉心回廊を守る古代管理機構（機界帯スケーリング）。
   ur_sentinel: 未記録の座標で「記録されないこと」を守る番人
   （現行最深の隠しBoss。撃破報酬も探索報酬級に持ち上げる）。 */
{
 const t=scale(BOSS_BASE,'炉心の監督機・FORGEWARDEN',22,{role:'boss',chapterId:'side_forgeheart'});
 t.hp=Math.round(t.hp*1.40);t.atk=Math.round(t.atk*1.12);t.def=Math.round(t.def*1.25);
 t.xp=Math.round(t.xp*1.7);t.gold=Math.round(t.gold*1.9);
 ENEMY_TYPES.fh_overseer=t;
}
{
 const t=scale(BOSS_BASE,'記録の番人・UNWRITTEN',41,{role:'boss',chapterId:'side_unrecorded'});
 t.hp=Math.round(t.hp*1.45);t.atk=Math.round(t.atk*1.18);t.def=Math.round(t.def*1.22);
 t.xp=Math.round(t.xp*1.8);t.gold=Math.round(t.gold*2.0);
 ENEMY_TYPES.ur_sentinel=t;
}
