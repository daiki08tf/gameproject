/* ============================================================
   Phase 11.1 — Story Canon
   ------------------------------------------------------------
   This file is the canonical narrative model for Blade Vale 3.0.
   It does not change combat/progression numbers. Story/UI layers should
   consume these meanings instead of inventing parallel explanations.
   ============================================================ */

export const STORY_CANON_VERSION = '3.0-11.1';
...
export const CENTRAL_MYSTERY = Object.freeze({
  ...
  truthPolicy: '答えを一度に説明しない。Ch1–15は違和感、Ch16–20はThe Veil、Ch21–25は境界網、機界は観測構造、Modern Worldは最後まで断片的に見せる。',
});

export const WORLD_LAYERS = Object.freeze({
  human: {
    id: 'human', name: '人界', kind: 'inhabited-world',
    role: 'プレイヤーの故郷と通常世界。複数の地域・文明・魔物生態系を持つ一つの世界層。',
    relation: 'The Veilの内側に隔離された観測対象の一層。本人たちは通常その事実を知らない。',
  },
  heaven: {
    id: 'heaven', name: '天界', kind: 'adjacent-world',
    role: '高位存在と光/Relic文化が残る隣接世界層。神話上の「天」だが絶対的な創造主の領域ではない。',
    relation: '人界と同じ境界網に属し、過去には人界へ干渉・裁定を試みた記録がある。',
  },
  underworld: {
    id: 'underworld', name: '冥界', kind: 'adjacent-world',
    role: '死・残響・高危険な生命圏が濃い隣接世界層。単純な死後世界ではなく、失われた記憶や存在の残滓が集積する。',
    relation: '人界・天界と同じ境界網の別層。Veilの損傷で相互流入が起こりやすくなる。',
  },
  boundary: {
    id: 'boundary', name: '境界層', kind: 'infrastructure',
    role: '世界と世界の間にある通路・隔壁・観測・封印のための中間領域。',
    relation: 'The Veilはこの境界層を束ねる巨大な隔離・接続網の総称。Key Dungeon、裂界、境界王座は同系統の構造物。',
  },
  machine: {
    id: 'machine', name: '機界', kind: 'artificial-world',
    role: '境界網の運用・記録・選別に関与する人工世界層。MOTHER/ARCHITECT系は管理端末であり、世界そのものの創造主ではない。',
    relation: '人界を「観測対象」「実験層」と呼ぶ。最深部では、機界自身も上位の観測者に観測される側だと判明する。',
  },
  modern: {
    id: 'modern', name: '現代世界', kind: 'external-world',
    role: 'Blade Vale側から見れば未知の文明世界。都市光、鉄道音、通信信号、文字、日付、携帯端末など断片だけが先に届く。',
    relation: '境界網の外部、または通常の管理対象とは異なる接続先。なぜ接続しているかはPhase 11後半以降の中心謎として保持する。',
  },
});
...
export const VEIL_CANON = Object.freeze({
  ...
  keys: '七鍵は既知の境界制御系。第八鍵は既存体系から外れた接続経路で、機界・外部観測層へ到達する異常な鍵。',
});
...
export const STORY_ARCS = Object.freeze([
  ...
  {id:'arc4',chapters:null,name:'第八鍵と機界',purpose:'既知の七鍵体系から外れた接続を追い、人工世界と観測者の痕跡へ進む。',reveal:'管理者も観測される側'},
  {id:'arc5',chapters:null,name:'現代世界の信号',purpose:'機界のさらに外側から届く人工信号を追う。現代世界の存在は見せるが、接続理由の最終回答は急がない。',reveal:'二つの世界は意図的または歴史的に接続されている可能性'},
]);
...
export function storyCanonSummary(){
  return {
    ...
    centralMystery:CENTRAL_MYSTERY.question,
    objective:CENTRAL_MYSTERY.playerObjective,
    veil:VEIL_CANON.definition,
    worlds:Object.values(WORLD_LAYERS).map(w=>w.name),
    arcCount:STORY_ARCS.length,
  };
}
