...
function executeOffensiveSkill(engine,companion,skill){const target=chooseTarget(engine,companion,skill);if(!target)return null;companion.mp-=skill.mpCost||0;const damage=companionDamage(companion,target,skill),kill=engine._applyRawDamageAndReward(target,damage),debuff=target.dead?null:applySkillDebuff(target,skill);const lifesteal=traitEffect(companion,'lifesteal');if(lifesteal&&damage>0)companion.hp=Math.min(companion.maxHp,companion.hp+Math.max(1,Math.round(damage*lifesteal.power)));return{action:'skill',companion:true,companionId:companion.id,companionName:companion.name,name:`${companion.name}の${skill.name}`,techType:skill.type==='debuff'?'damage':skill.type,targets:[{targetId:target.id,targetName:target.name,damage,critical:false,defeated:target.dead,effects:debuff?[debuff]:[],kill}]};}
