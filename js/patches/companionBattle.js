...
// Boss戦で仲間3体が完全な無料DPSにならないための巻き込み設計。
// Boss本体の予兆/フェーズ/技選択はBattleEngineへ任せたまま、解決済みの攻撃に
// 対してだけ仲間側へ被害を追加するのでBoss AIの順序を壊さない。
...
function performCompanionTurn(engine, companion = null) {
  ensureCompanionBattle(engine);const c=companion||engine.companion;if(!c||c.down||c.hp<=0||engine.over)return null;
  const species=getCompanionSpecies(c.speciesId);if(!species)return null;
  const skill=chooseCompanionSkill(species, c, engine.aliveEnemies);if(!skill)return null;
  if(skill.type === 'heal')return executeHeal(c,skill);return executeOffensiveSkill(engine, c, skill);
}
function companionCanBeTargeted(engine){return livingCompanions(engine).length>0;}
function applyDamageToCompanion(enemy,c,mult=1){const damage=enemyDamageToCompanion(enemy,c,mult);c.hp=Math.max(0,c.hp-damage);if(c.hp<=0)c.down=true;return{enemyId:enemy.id,name:enemy.name,kind:'attack',damage,evaded:false,companionTarget:true,companionId:c.id,companionName:c.name,companionHp:c.hp,companionMaxHp:c.maxHp,companionDown:c.down};}
function hitCompanion(engine,enemy,mult=1){const alive=livingCompanions(engine);if(!alive.length)return null;const c=alive[Math.floor(Math.random()*alive.length)];return applyDamageToCompanion(enemy,c,mult);}
...
function bossCompanionCollateral(engine,enemy,result){
  ...
  if(result.kind==='special'){
    const mult=bossSpecialBaseMult(result.specialKind)*BOSS_COMPANION_COMBAT.SPECIAL_SPLASH_MULT;
    return [...livingCompanions(engine)].map(c=>applyDamageToCompanion(enemy,c,mult));
  }
  if(result.kind==='attack'&&Math.random()<BOSS_COMPANION_COMBAT.BASIC_CLEAVE_CHANCE){
    const hit=hitCompanion(engine,enemy,BOSS_COMPANION_COMBAT.BASIC_CLEAVE_MULT);return hit?[hit]:[];
  }
  return [];
}
function withCompanionEnemyDebuff(enemy, fn) {
  ...
  const originalAtk = enemy.atk;enemy.atk=Math.max(1,Math.round(originalAtk*enemy._companionAtkDebuffMult));
...
BattleEngine.prototype.performEnemyTurn=function patchedPerformEnemyTurn(enemy){
  ...
  return withCompanionEnemyDebuff(enemy,()=>{
    if(!enemy.dead&&!enemy.boss&&companionCanBeTargeted(this)){
      const candidates=livingCompanions(this),defensive=candidates.filter(c=>(COMPANION_NATURES[c.nature]||COMPANION_NATURES.balanced).ai==='defensive'),targetChance=.28+(defensive.length?.07:0);
      if(Math.random()<targetChance)return hitCompanion(this,enemy);
    }
    const result=originalPerformEnemyTurn.call(this,enemy);
    if(enemy?.boss&&result)result.companionSplash=bossCompanionCollateral(this,enemy,result);
    return result;
  });
};
...
function convertCompanionCombatEvent(event){
  if(!event||event.type!=='enemyAction'||!event.result)return[event];
  const r=event.result;
  if(r.companionTarget)return[companionHitLog(r)];
  const splash=Array.isArray(r.companionSplash)?r.companionSplash.filter(Boolean):[];
  if(!splash.length)return[event];
  return[event,...splash.map(x=>companionHitLog(x,r.kind==='special'?'特殊攻撃の余波':'攻撃の巻き込み'))];
}
...
TextBattleScreen.prototype._render=function patchedCompanionRender(){originalRender.call(this);const el=ensureCompanionHud(this);if(!el)return;const synergy=this.engine.companionSynergies?.length?`<div>🔗 ${this.engine.companionSynergies.map(s=>s.name).join(' / ')}</div>`:'';el.innerHTML=synergy+this.engine.companions.map((c,i)=>{const down=c.down||c.hp<=0;return `<div style="opacity:${down?.55:1}">🐾${i+1} ${c.name} Lv.${c.level}　HP ${Math.max(0,c.hp)}/${c.maxHp}　MP ${Math.max(0,c.mp)}/${c.maxMp}${down?'　【戦闘不能】':''}</div>`;}).join('');};
export { performCompanionTurn,effectiveCompanionSpd,companionDamage,enemyDamageToCompanion,applySkillDebuff,livingCompanions,bossCompanionCollateral,companionNatureDamageMult };
