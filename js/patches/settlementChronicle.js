/* Settlement 3.0 S17 — Monuments, Museum & Chronicle. Read-only derived view. */
...
state.settlementChronicle=function(){
 const bosses=bossRecords(),uniques=uniqueRecords(),generations=generationRecords(),tier=this.activeWorldTier?.()||{id:this.data.worldTierId||'normal',name:'Normal',rank:0},abyssBest=Math.max(0,Number(this.data.abyssBestDepth)||0),currentGeneration=Math.max(1,(Number(this.data.reincarnations)||0)+1);
 const rows={
...
state.settlementChronicleSummary=function(){const exhibits=this.settlementChronicle();return{exhibits:exhibits.length,bosses:exhibits.find(x=>x.id==='bosses')?.records.length||0,uniques:exhibits.find(x=>x.id==='uniques')?.records.length||0,generation:Math.max(1,(Number(this.data.reincarnations)||0)+1),abyssBestDepth:Math.max(0,Number(this.data.abyssBestDepth)||0),worldTier:this.activeWorldTier?.()||null};};
