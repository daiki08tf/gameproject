...
function questCard(quest){
 ...
 return `<div class="forge-card" data-settlement-secret-quest="${quest.id}" style="padding:10px 12px;"><div class="forge-card-top"><div class="forge-card-name">${quest.name}</div><strong>${status}</strong></div><div class="forge-card-sub">${body}</div>${!done&&!pending&&current?`<div class="forge-card-sub" style="margin-top:5px;">次：${current.title}</div>`:''}<button class="forge-card-btn settlement-secret-advance" data-quest="${quest.id}" ${done||pending?'disabled':''} style="margin-top:8px;">${action}</button></div>`;
}
export function renderSettlementSecrets(){
 ...
 section.querySelectorAll('.settlement-secret-advance').forEach(btn=>btn.addEventListener('click',()=>{const r=state.advanceSettlementSecretQuest?.(btn.dataset.quest);if(!r?.ok)return;const reward=rewardText(r.gained);alert(`${r.quest.name}\n${r.step.title}\n${r.step.text}${r.pendingEncounter?`\n遭遇: ${r.encounter.name}`:''}${reward?`\n獲得: ${reward}`:''}`);section.remove();renderSettlementSecrets();}));
}
