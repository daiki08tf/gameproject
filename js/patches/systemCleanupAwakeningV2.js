...
state.awakeningV2Rank = function awakeningV2Rank() { ensureCleanupData(); return this.data.awakeningV2Rank; };
...
state.artifactSlotCount = function artifactSlotCountV2() { return Math.min(3, this.awakeningV2Rank()); };
state.isAwakeningFeatureUnlocked = function isAwakeningFeatureUnlocked(feature) {
  const rank = this.awakeningV2Rank();
  const gates = { awakenedEquipment: 1, artifact: 1, extremeAffix: 2, relic: 3, abyssAscension: 4 };
  return rank >= (gates[feature] || 99);
};

state.awakenWeaponCost = function awakenWeaponCostV2(rank) { return 20 + Math.max(0, Math.floor(Number(rank) || 0)) * 15; };
...
// Progression 3.0: Relicは覚醒Rank3だけで一括購入できず、深淵Era到達も必要。
// 既に解放済みの旧セーブは没収しない（isArtifactUnlockedが先に成立する）。
state.artifactProgressionGate = function artifactProgressionGate(id) {
  const artifact = getArtifact(id);
  if (!artifact) return { met:false, reason:'unknown' };
  const isRelic = !!artifact.kind;
  const awakeningRequired = isRelic ? 3 : 1;
  const rankMet = this.awakeningV2Rank() >= awakeningRequired;
  const depthRequired = isRelic ? Math.max(0, Math.floor(Number(artifact.abyssDepthRequired) || 0)) : 0;
  const bestDepth = Math.max(0, Math.floor(Number(this.data.abyssBestDepth) || 0));
  const depthMet = bestDepth >= depthRequired;
  return {
    met: rankMet && depthMet,
    rankMet,
    awakeningRequired,
    depthMet,
    depthRequired,
    bestDepth,
    era: artifact.progressionEra || null,
  };
};
state.canUnlockArtifact = function canUnlockArtifactV2(id) {
  ...
  if (!this.artifactProgressionGate(id).met) return false;
