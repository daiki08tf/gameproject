...
state.markEnemyAnalyzed=function(enemy){const e=entryFor(enemy);if(!e)return false;e.analyzed=true;e.behaviorKnown=true;e.roleKnown=true;const p=enemyCombatProfile(idOf(enemy));e.observedSkills=Array.isArray(e.observedSkills)?e.observedSkills:[];if(p.skill?.name&&!e.observedSkills.includes(p.skill.name))e.observedSkills.push(p.skill.name);this.save();return true;};
state.enemyKnowledge=function(enemyId){const e=codex()[enemyId]||{},p=enemyCombatProfile(enemyId),r=enemyRole(enemyId);return{seen:!!e.seen,kills:e.kills||0,roleKnown:!!e.roleKnown||!!e.analyzed,behaviorKnown:!!e.behaviorKnown||!!e.analyzed,analyzed:!!e.analyzed,role:r,skill:p.skill,observedSkills:Array.isArray(e.observedSkills)?e.observedSkills:[]};};
