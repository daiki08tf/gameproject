...
function regionCard(bundle) {
  const { region, faunaSeen, faunaTotal, fishingSpot, fishSeen, fishTotal, archaeologySite, treasureHunts } = bundle;
  const lines = [];
...
function render() {
  ...
  box.innerHTML = `<div class="forge-card-name">地域の記録</div><div class="sub">生物・釣果・発掘・宝探し ―― この地で分かっていることを、地域ごとにまとめて確認できる。</div>${bundles.map(regionCard).join('')}`;
