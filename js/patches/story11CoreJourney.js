/* Phase 11.2 — Core journey story integration.
...
function attachCoreStory(){
  for(const chapter of CHAPTERS){
    if(chapter.num<1||chapter.num>15)continue;
    const mainStages=chapter.stages.filter(stage=>!stage.branch&&!stage.bounty);
    chapter.stages.forEach(stage=>{
      const mainIndex=mainStages.indexOf(stage);
      if(mainIndex<0)return;
      const beat=coreStoryBeatForStage(chapter.num,stage,mainIndex,mainStages.length);
...
function storyStartLines(stage){
  ...
  const lines=[];
  if(story.opening)lines.push(`【旅の目的】${story.objective}`,story.opening);
...
export { attachCoreStory, storyStartLines, storyClearLines };
