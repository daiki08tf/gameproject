...
import { recordEnemy2Discovery, isEnemy2GeneratedMaterializationId } from '../data/enemyCodex2Discovery.js';
...
state.markCodexSeen=function(enemy,stage=null){const id=enemyCodexId(enemy);if(!id)return;const entries=ensureCodex(),e=ensureCodexEntry(entries,id,enemy.name||id);e.seen=true;if(enemy?.rank==='rare'||enemy?.rareIdentity)e.rare=true;recordEnemy2Discovery(entries,enemy,stage);state.save();};
state.markCodexKill=function(enemy,stage=null){const id=enemyCodexId(enemy);if(!id)return;const entries=ensureCodex(),e=ensureCodexEntry(entries,id,enemy.name||id);e.seen=true;e.kills=(e.kills||0)+1;if(enemy?.rank==='rare'||enemy?.rareIdentity)e.rare=true;recordEnemy2Discovery(entries,enemy,stage,{kill:true});state.save();};
