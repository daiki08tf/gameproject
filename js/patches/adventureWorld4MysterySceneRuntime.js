...
const SHORTCUT='mystery:shortcutId';
...
state.adventure4ContentPackIScene=function(){
  ...
  if(session.temporaryFlags?.[SHORTCUT]){activeMysterySceneId='frontier-bell-vault-revisit';return adventure4MysterySecretRevisitScene();}
  if(session.regionId==='frontier'&&!session.temporaryFlags?.[DONE]){
    const stage=this.adventure4MysteryStage?.('frontier-buried-bell');const scene=adventure4MysterySceneForStage(stage);
...
state.completeAdventure4ContentPackIScene=function(){
  if(activeMysterySceneId){const id=activeMysterySceneId,shortcut=id==='frontier-bell-vault-revisit';activeMysterySceneId=null;patchFlags(this,{[DONE]:true,[SHORTCUT]:shortcut?null:this.adventure4Session()?.temporaryFlags?.[SHORTCUT]||null});return{ok:true,mysterySceneId:id};}
