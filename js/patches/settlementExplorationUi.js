...
export function renderSettlementExploration(){
 ...
 section.querySelectorAll('.settlement-explore-location').forEach(btn=>btn.addEventListener('click',()=>{const r=state.exploreSettlementLocation?.(btn.dataset.location);if(!r?.ok)return;const reward=rewardText(r.gained);alert(`${r.location.name}\n${r.event}${reward?`\n獲得: ${reward}`:''}`);section.remove();renderSettlementExploration();}));
}
