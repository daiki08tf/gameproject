...
function render(){
 ...
 box.innerHTML=`<summary>記念碑・博物館・年代記　${exhibits.map(x=>`${esc(x.value)}`).join('　')}</summary>
 <div class="forge-card-sub" style="padding:8px;margin-top:6px">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:6px;margin-top:7px">${exhibits.map(x=>`<div style="border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:7px"><strong>${esc(x.place)}</strong><div>${esc(x.name)}：${esc(x.value)}</div><small>${esc(x.detail)}</small></div>`).join('')}</div>
