...
function render(){
 ...
 section.innerHTML=`<details class="forge-card"><summary>🌐 終端ネットワーク ${summary.online||0}/${summary.total||nodes.length}接続${summary.attention?` ・ ${summary.attention}件注目`:''}</summary><div class="forge-card-sub" style="margin:8px 0;">World Tierや各Endgameの報酬倍率は正本側のまま。Settlementは状況・研究・導線だけを統合する。</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:8px;">${nodes.map(n=>`<div class="forge-card" style="padding:9px 10px;"><div class="forge-card-top"><div class="forge-card-name">${n.icon} ${n.name}</div><strong>${n.status}</strong></div><div class="forge-card-sub">${n.area}｜${n.detail}</div>${n.attention?'<div class="forge-card-sub" style="margin-top:4px;">● 対応・確認候補</div>':''}</div>`).join('')}</div><button class="forge-card-btn settlement-abyss-return" style="margin-top:8px;">深層帰還報告を確認</button></details>`;
