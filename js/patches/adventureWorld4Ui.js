...
import { adventure4Clr3RunSummary } from '../data/coreLoopClr3.js';
...
function clrRouteOptions(region){return{worldTierRank:state.adventure4HighLevelStateForRegion?.(region?.id)?.worldTierRank||0};}
...
function adventureSummaryText(current,session,route){
  if(current.tags?.includes('clr2-aftermath')||current.tags?.includes('clr2-steady')||current.tags?.includes('clr2-pressure')){
    const run=adventure4Clr3RunSummary(session,route);
    const guidance=current.tags?.includes('clr2-steady')
      ?'追加強敵を避け、深部へ急ぎます。'
      :current.tags?.includes('clr2-pressure')?'追加強敵を狩るぶん、通常の戦闘報酬をもう1回狙えます。':'ここで装備と残存戦力を確認し、続行か帰還を判断できます。';
    return `${run.progressLabel} / ${run.routeLabel} / ${run.remainingLabel}。${guidance}`;
  }
  if(['battle','elite','boss'].includes(current.type))return'既存Stageの戦闘へ接続します。EXP・Gold・Lootは通常の進行・報酬処理へそのまま反映されます。';
  return'次に進む道を選びます。見えているのは直近の行き先だけです。';
}
...
export function renderAdventureRoute(){
  ...
  const summary=document.createElement('section');summary.className='adventure4-card adventure4-current';summary.innerHTML=`<div class="adventure4-meta">${region.name}</div><h3>${current.name}</h3><p>${adventureSummaryText(current,session,route)}</p>`;body.appendChild(summary);
  ...
  const latest=state.adventure4Session(),pending=latest.pendingEncounter;
  const pendingCurrent=pending?.stageId&&pending.nodeId===current.id;
  if(pendingCurrent){const retry=document.createElement('button');retry.className='adventure4-choice adventure4-battle';retry.innerHTML='<span><strong>中断した戦闘へ戻る</strong><small>戦闘</small></span><span>›</span>';retry.addEventListener('click',()=>launchAdventureBattle({...current,stageId:pending.stageId}));body.appendChild(retry);}
  const currentCombat=['battle','elite','boss'].includes(current.type);
  if(currentCombat&&!pendingCurrent){const start=document.createElement('button');start.type='button';start.className=`adventure4-choice adventure4-${current.type}`;const label=state.isStageCleared?.(current.stageId)?'再戦する':'戦闘開始';start.innerHTML=`<span><strong>${label}</strong><small>${current.type==='battle'?'戦闘':'強敵'}</small></span><span>›</span>`;start.addEventListener('click',()=>{Audio_.tap();launchAdventureBattle(current);});body.appendChild(start);}
