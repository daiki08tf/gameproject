...
function renderCompanionScreen(){
  ...
  const partySummary=party.map((id,i)=>{const c=id?state.getCompanion(id):null;return `<div class="forge-card-sub">枠${i+1}: ${c?`${c.species.icon||'🐾'} ${escapeHtml(c.instance.nickname||c.species.name)} Lv.${c.instance.level}`:'－'}</div>`;}).join('');
