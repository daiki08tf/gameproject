...
const SAVE_FIELDS = { companionInstances: {}, companionParty: [null], nextCompanionSeq: 1, companionCodex: {} };
...
state.setActiveCompanion=function setActiveCompanion(instanceId){if(instanceId!=null&&!this.data.companionInstances[instanceId])return false;this.data.companionParty[0]=instanceId||null;this.save();return true;};
...
function renderCompanionScreen(){const content=document.getElementById('companionContent');if(!content)return;const activeId=state.activeCompanionId(),list=state.companionList();if(!list.length){content.innerHTML='<p class="hint">まだ仲間はいません。</p>';return;}content.innerHTML=list.map(({id,species,instance,stats})=>{const active=id===activeId,nature=COMPANION_NATURES[instance.nature]||COMPANION_NATURES.balanced,xpNeed=companionExpToNext(instance.level),displayName=escapeHtml(instance.nickname||species.name),traits=(species.traits||[]).map(companionTraitLabel).join('・')||'なし',skills=unlockedCompanionSkills(species,instance.level),skillText=skills.length?skills.map(s=>`${s.name}${s.mpCost?` MP${s.mpCost}`:''}: ${s.desc}`).join(' / '):'なし',evo=state.companionEvolutionInfo?.(id),evoText=evo?`進化: ${evo.name}（Lv.${evo.level} / ${COMPANION_RARITY_LABEL[evo.minRarity]}以上）`:instance.evolution?'進化済み':'進化先なし';return `<div class="forge-card companion-card ${active?'companion-active':''}" data-companion-id="${escapeHtml(id)}"><div class="forge-card-name">${species.icon||'🐾'} ${displayName} ${active?'【同行中】':''}</div><div class="forge-card-sub">Lv.${instance.level} / ${COMPANION_RARITY_LABEL[instance.rarity]} ${rarityStars(instance.rarity)} / 性格: ${escapeHtml(nature.name)}</div><div class="forge-card-sub">HP ${stats.hp}　MP ${stats.mp}　ATK ${stats.atk}　DEF ${stats.def}　MAG ${stats.mag}　SPD ${stats.spd}</div><div class="forge-card-sub">EXP ${instance.exp} / ${xpNeed}</div><div class="forge-card-sub">特性: ${escapeHtml(traits)}</div><div class="forge-card-sub">技: ${escapeHtml(skillText)}</div><div class="forge-card-sub">${escapeHtml(evoText)}</div><div class="confirm-actions" style="margin-top:8px;"><button class="btn-sub companion-set-btn" data-id="${escapeHtml(id)}" ${active?'disabled':''}>${active?'同行中':'同行させる'}</button>${evo?`<button class="btn-highlight companion-evolve-btn" data-id="${escapeHtml(id)}" ${evo.canEvolve?'':'disabled'}>進化</button>`:''}<button class="btn-sub companion-release-btn" data-id="${escapeHtml(id)}">${active?'同行解除して帰す':'帰す'}</button></div></div>`;}).join('');
  content.querySelectorAll('.companion-set-btn').forEach(btn=>btn.addEventListener('click',()=>{state.setActiveCompanion(btn.dataset.id);renderCompanionScreen();}));
