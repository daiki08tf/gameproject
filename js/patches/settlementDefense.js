...
state.upgradeSettlementDefense=function(id){const check=this.canUpgradeSettlementDefense(id);if(!check.ok)return check;for(const [k,v] of Object.entries(check.cost))this.data.settlementMaterials[k]-=v;const meta=ensureMeta();meta.projects[id]=check.next;this.save();return{ok:true,id,level:check.next};};
state.settlementDefenseIncidents=function(){const meta=ensureMeta(),ctx=context();return SETTLEMENT_INVASIONS.map(i=>({...i,available:settlementDefenseIncidentEligible(i,ctx),cleared:meta.cleared.includes(i.id),attempts:Number(meta.attempts[i.id]||0),pending:meta.pending===i.id}));};
...
state.resolveSettlementDefense=function(id,cleared){const meta=ensureMeta(),i=invasion(id);if(!i||meta.pending!==id)return{ok:false,reason:'noPending'};meta.pending=null;if(!cleared){this.save();return{ok:true,cleared:false,incident:i,buildingLoss:false};}const first=!meta.cleared.includes(id);if(first)meta.cleared.push(id);const gained=first?(this.addSettlementMaterials?.(i.firstReward||{})||{}):{};this.save();return{ok:true,cleared:true,first,incident:i,gained,buildingLoss:false};};
