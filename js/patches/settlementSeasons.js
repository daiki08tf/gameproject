...
import { SETTLEMENT_SEASONS,SETTLEMENT_FESTIVALS,settlementFestivalEligible } from '../data/settlementSeasons.js';
import { weatherForClimate,daypartForCycle } from '../data/worldClimate.js';
...
function snapshot(){const s=ensure(),c=Math.max(0,Math.floor(s.cycle));const season=SETTLEMENT_SEASONS[Math.floor(c/4)%SETTLEMENT_SEASONS.length],weather=weatherForClimate('default',c),daypart=daypartForCycle(c);const hall=state.settlementLevel?.('hall')||0;const festival=SETTLEMENT_FESTIVALS.find(f=>settlementFestivalEligible(f,{hall,season:season.id}))||null;return{cycle:c,season,weather,daypart,festival};}
