...
export function renderSettlementRanch3(){
  ...
  const caps=summary.capabilities.map(c=>`<span style="display:inline-block;margin:2px;padding:4px 7px;border:1px solid rgba(255,255,255,.18);border-radius:999px;opacity:${c.unlocked?1:.48};">${esc(c.name)} ${c.unlocked?'✓':'LOCK'}</span>`).join('');
