...
function mysteryCard(view){
  const label={unknown:'未着手',rumor:'噂',trace:'痕跡',discovery:'発見',research:'研究済み',resolved:'解決'}[view.stage]||view.stage;
  const canResearch=view.stage==='discovery';
  return `<div class="forge-card" data-mystery="${view.id}" style="padding:10px 12px;"><div class="forge-card-top"><div class="forge-card-name">🕯️ ${view.name}</div><strong>${label}</strong></div><div class="forge-card-sub">${view.hint||'記録を整理中'}</div>${canResearch?`<button class="forge-card-btn mystery-research" data-mystery-id="${view.id}" style="margin-top:8px;">証拠を研究する</button>`:''}${view.secretVisible?'<div class="forge-card-sub" style="margin-top:7px;">再探索できる地点が絞り込まれた。秘密の場所の存在はここで初めて確定する。</div>':''}</div>`;
}
function npcCard(npc){const role={merchant:'Merchant',traveler:'Traveler',scholar:'Scholar',tamer:'Tamer'}[npc.role]||npc.role;const place=npc.location==='settlement'?'Settlement':npc.location;return `<div class="forge-card" data-npc="${npc.id}" style="padding:9px 11px;"><div class="forge-card-top"><div class="forge-card-name">🧳 ${npc.name}</div><strong>${role}</strong></div><div class="forge-card-sub">現在地: ${place} ／ 遭遇 ${npc.meetings||0}回</div></div>`;}
...
export function renderAdventure4InvestigationBoard(){
  ...
  const mysteries=ADVENTURE4_MYSTERIES.map(m=>state.adventure4MysteryView?.(m.id)).filter(Boolean);
  const npcs=state.adventure4NpcNetwork?.()||[];
  ...
  const mysteryHtml=mysteries.map(mysteryCard).join('');const npcHtml=npcs.filter(n=>n.meetings>0||n.location==='settlement').map(npcCard).join('');
