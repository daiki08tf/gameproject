...
export function renderAdventure4InvestigationBoard(){
  ...
  panel.innerHTML=`<details class="forge-card" open><summary>調査記録 — 情報をつなぐ</summary><div class="forge-card-sub" style="margin:8px 0 10px;">持ち帰った証拠を結び付ける。Mysteryは必須Storyから独立し、分からなくなった時だけ次のヒントを示す。</div>${evidence}<div class="forge-card-name" style="margin:12px 0 7px;">長期Mystery</div><div style="display:grid;gap:7px;">${mysteryHtml}</div>${npcHtml?`<details class="forge-card" style="margin-top:9px;"><summary>旅人ネットワーク</summary><div style="display:grid;gap:7px;margin-top:8px;">${npcHtml}</div></details>`:''}</details>`;
