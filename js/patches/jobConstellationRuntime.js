...
state.fusionNodeStatus=function(fusionId,nodeId){
 const nodes=fusionConstellationFor(fusionId),n=nodes.find(x=>x.id===nodeId);if(!n)return{exists:false,bought:false,canBuy:false};
 const b=this.fusionPurchased(fusionId),requires=(n.requires||[]).every(id=>b.has(id)),requiresAny=!n.requiresAny?.length||n.requiresAny.some(id=>b.has(id));
 let exclusiveOk=true;if(n.exclusiveGroup){exclusiveOk=!nodes.some(x=>x.id!==n.id&&x.exclusiveGroup===n.exclusiveGroup&&b.has(x.id));}
 const prereq=requires&&requiresAny&&exclusiveOk,discovered=this.isFusionDiscovered(fusionId),canBuy=discovered&&!b.has(n.id)&&prereq&&this.fusionPointsAvailable(fusionId)>=n.cost;
 return{exists:true,bought:b.has(n.id),prereq,requires,requiresAny,exclusiveOk,discovered,canBuy,node:n};
};
...
// Non-destructive migration: old three-node Fusion saves remain valid. If an old
// Keystone/Ultimate was purchased before branching existed, keep it active;
// future purchases must follow one of the new paths.
