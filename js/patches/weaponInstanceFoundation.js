...
// ---------- Item Power ----------
// A comparison/QoL score only. Combat formulas are NOT changed here.
// It intentionally values both raw stats and affix power so auto-equip
// no longer ignores a strong affix roll on a slightly weaker base weapon.
function effectPowerScore(effects) {
  ...
  for (const e of effects || []) {
    ...
    // Passive effects are consistently valuable; proc effects are weighted
    // by chance so a rare proc does not dominate the score by itself.
...
function statAffixScore(statBonus) {
  ...
  for (const [stat, value] of Object.entries(statBonus || {})) {
    // Percentage-like affix values are stored as fractions.
...
state.enhanceMaterialCandidates = function enhanceMaterialCandidates(targetId) {
  ...
  for (const [id, qty] of Object.entries(this.data.inventory)) {
    ...
    // A concrete instance being enhanced from the bag must survive.
    // Legacy non-instance IDs remain stack-based and preserve old behavior.
    const reserve = this.isWeaponInstance(targetId) && id === targetId ? 1 : 0;
    for (let i = reserve; i < qty; i++) out.push(id);
  }

  // Consume the least valuable spare copies first. Good affix rolls are
  // therefore much less likely to be eaten accidentally.
...
// Exported only for lightweight/manual regression checks if needed.
