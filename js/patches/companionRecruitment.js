...
BattleEngine.prototype._grantKillRewards = function patchedRecruitGrantKillRewards(enemy) {
  ...
  if (speciesId) {
    this._recruitDefeats.push({ speciesId, enemyType: enemy.type, enemyName: enemy.name, elite: !!enemy.elite });
  }
...
function rollRecruitCandidate(engine) {
  ...
  for (const entry of rolls) {
    ...
    if (Math.random() < chance) {
      return { speciesId: species.id, name: species.name, icon: species.icon || '🐾', chance, elite: entry.elite };
    }
  }
  return null;
}
...
function showRecruitPrompt(candidate, onDone) {
  ...
  const finish = (accepted) => {
    ...
    if (accepted && state.createCompanion) {
      const opts = candidate.elite ? { minRarity: 'rare', origin: 'eliteRecruit' } : { origin: 'recruit' };
...
TextBattleScreen.prototype.start = function patchedRecruitStart(stageId, onEnd, blessingId) {
  const wrappedOnEnd = (result) => {
    if (!result || !result.cleared || !result.recruitCandidate) { onEnd(result); return; }
    showRecruitPrompt(result.recruitCandidate, (recruitResult) => {
