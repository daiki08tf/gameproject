...
function startBattle(stage,blessingId){lastStageId=stage.id;showScreen('textBattleScreen');battle.start(stage.id,result=>{if(result.cleared&&state.rollRune2DropForStage)result.rune2Drops=state.rollRune2DropForStage(stage.id);else result.rune2Drops=[];renderResult(result);showScreen('resultScreen');},blessingId);}
...
document.getElementById('weaponCodexBackBtn').addEventListener('click',()=>{Audio_.tap();renderEquipment();showScreen('equipmentScreen');});initWeaponCodexTabs();
document.getElementById('jobsBackBtn').addEventListener('click',()=>{Audio_.tap();goHome();});
document.getElementById('blacksmithBackBtn').addEventListener('click',()=>{Audio_.tap();goHome();});initBlacksmithTabs();
document.getElementById('rebirthBackBtn').addEventListener('click',()=>{Audio_.tap();goHome();});initRebirthTabs();
...
document.getElementById('resultRetryBtn').addEventListener('click',()=>{Audio_.tap();if(lastStageId){const found=pendingStage&&pendingStage.id===lastStageId?pendingStage:null;startBattle(found||pendingStage,getSelectedBlessingId());}});
