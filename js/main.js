...
const battle=new TextBattleScreen();let pendingStage=null,lastStageId=null,currentChapterIndex=0,cameFromAbyss=false,nextResultStage=null;
...
function ensureNextStageButton(){let btn=document.getElementById('resultNextBtn');if(btn)return btn;const actions=document.querySelector('#resultScreen .confirm-actions');if(!actions)return null;btn=document.createElement('button');btn.id='resultNextBtn';btn.className='btn-main hidden';btn.textContent='次のステージへ';actions.appendChild(btn);btn.addEventListener('click',()=>{if(!nextResultStage)return;Audio_.tap();pendingStage=nextResultStage;cameFromAbyss=!!nextResultStage.isAbyss;startBattle(nextResultStage,getSelectedBlessingId());});return btn;}
function updateNextStageButton(result,stage){const pacts=stage?.isAbyss&&state.activeAbyssPacts?state.activeAbyssPacts((stage.abyssDepth||0)+1):[];nextResultStage=result?.cleared?nextStageAfter(stage,pacts):null;const btn=ensureNextStageButton();if(!btn)return;btn.classList.toggle('hidden',!nextResultStage);if(nextResultStage)btn.textContent=nextResultStage.isAbyss?`次の階へ (${nextResultStage.abyssDepth}F)`:'次のステージへ';}
function startBattle(stage,blessingId){lastStageId=stage.id;showScreen('textBattleScreen');battle.start(stage.id,result=>{result.rune2Drops=result.cleared&&state.rollRune2DropForStage?state.rollRune2DropForStage(stage.id):[];renderResult(result);updateNextStageButton(result,stage);showScreen('resultScreen');},blessingId);}
