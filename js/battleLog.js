...
const SPECIAL_FLAVOR = { slam: '叩きつけ', charge: '突進', projectile: '遠距離攻撃' };
const STAT_JP = { atk: '攻撃力', def: '防御力', spd: '素早さ' };
function affinityTag(tier){return {weakMajor:'【大弱点！】',weak:'【弱点！】',resistMajor:'【強耐性】',resist:'【耐性】'}[tier]||'';}
...
function describeEffectEvent(ev) {
  switch (ev.kind) {
    ...
    case 'everyNHits': return ev.aoe ? `渾身の一撃が周囲に炸裂！${ev.hits.join('、')}に${ev.amount}ずつダメージ！` : `渾身の一撃！さらに${ev.amount}の追加ダメージ！`;
...
function describeKill(kill, targetName) {
  ...
  for (const ev of kill.onKillEvents || []) { const d = describeEffectEvent(ev); if (d) lines.push(d); }
  for (const d of kill.drops || []) lines.push(`${d.isNew ? '【NEW】' : ''}${d.name}を手に入れた！`);
...
function describeHitEffects(effects) {
  ...
  for (const ev of effects || []) {
    const d = describeEffectEvent(ev); if (d) lines.push(d);
...
function describePlayerAction(result) {
  ...
  if (result.action === 'guard') { lines.push('ぼうぎょの構えを取った！ 次に受ける攻撃のダメージが下がる！'); return lines; }
    ...
    switch (result.techType) {
      case 'damage':
        for (const hit of result.targets || []) {
          ...
          lines.push(`${critTag}${hit.targetName}に${hit.damage}のダメージ！${affinityTag(hit.affinityTier)}`);
        ...
        break;
      case 'heal':
        lines.push(`HPを${result.healAmount}回復した！`); if (result.mpRestored) lines.push(`MPを${result.mpRestored}回復した！`); if (result.buffed) lines.push('身体能力が上がった！'); break;
      case 'buff':
        if (result.healAmount) lines.push(`HPを${result.healAmount}回復した！`); lines.push(result.telegraphBonusApplied ? '相手の構えを見切り、身体能力が大きく上がった！' : '身体能力が上がった！'); break;
      case 'debuff':
        for (const t of result.targets || []) {
          for (const w of t.weakenApplied || (t.weakenStat ? [{ stat: t.weakenStat }] : [])) lines.push(`${t.targetName}の${STAT_JP[w.stat] || w.stat}を弱体化させた！`);
      ...
      case 'steal':
        ...
        else { lines.push(`ゴールドを${result.stolenGold}奪った！`); if (result.stolenItem) lines.push(`${result.stolenItem.name}を奪い取った！`); }
        break;
      case 'inspect':
        if (result.inspected) { const t = result.inspected; lines.push(`${t.name}：HP ${t.hp}/${t.maxHp}　ATK ${t.atk}　DEF ${t.def}　SPD ${t.spd}`); }
        else lines.push('しかし、狙う相手がいなかった！');
        break;
      case 'burst':
        for (const t of result.targets || []) { const critTag = t.critical ? '会心の一撃！ ' : ''; lines.push(`${critTag}${t.targetName}に${t.damage}のダメージ！（${t.consumedStacks}スタック分の毒を起爆！）`); if (t.defeated) lines.push(...describeKill(t.kill, t.targetName)); }
        if ((result.targets || []).length === 0) lines.push('しかし、狙う相手がいなかった！');
        break;
      case 'cleanse': lines.push('体を蝕むものが消え去った！'); break;
      case 'utility':
        if (result.buffed) lines.push('身体能力が上がった！');
        if (result.telegraphBonusApplied) lines.push('相手の構えを見切り、大きく防御を固めた！'); else lines.push('構えを固めた！');
...
function describeEnemyAction(result) {
  ...
  if (result.kind === 'telegraph') { lines.push(`${result.name}が${SPECIAL_FLAVOR[result.specialKind] || '大技'}の構えを見せた…！ 次のターンに来る！`); return lines; }
  if (result.kind === 'special') {
    ...
    lines.push(`${result.name}の${SPECIAL_FLAVOR[result.specialKind] || '大技'}が炸裂！ ${result.damage}のダメージを受けた！`); lines.push(...describeHitEffects(result.hurtEvents)); return lines;
  }
  if (result.kind === 'summon') {
    if (result.capped || (result.added || []).length === 0) lines.push(`${result.name}が手下を呼び出そうとしたが、これ以上は展開しきれないようだ！`);
    else lines.push(`${result.name}が手下を呼び出した！ ${result.added.join('、')}が加わった！`);
    return lines;
  }
  if (result.evaded) { lines.push(`${result.name}の攻撃！ しかし回避した！`); lines.push(...describeHitEffects(result.evadeEvents)); return lines; }
  lines.push(`${result.name}の攻撃！ ${result.damage}のダメージを受けた！`); lines.push(...describeHitEffects(result.hurtEvents)); return lines;
}

function describeEncounterStart(enemies) {
  const counts = new Map(); for (const e of enemies) counts.set(e.name, (counts.get(e.name) || 0) + 1);
  const parts = [...counts.entries()].map(([name, n]) => `${name}が${n}体`); const anyBoss = enemies.some((e) => e.boss);
...
export function describeRound(events) {
  ...
  for (const ev of events) {
    switch (ev.type) {
      ...
      case 'enemyWait': break;
      case 'dotTick': lines.push(`${ev.name}は炎に焼かれている！ ${ev.amount}のダメージ！`); if (ev.targetDead && ev.kill) lines.push(...describeKill(ev.kill, ev.name)); break;
      case 'autoTurret': { const critTag = ev.critical ? '会心の一撃！ ' : ''; lines.push(`自動砲台が着弾！ ${critTag}${ev.targetName}に${ev.damage}のダメージ！`); if (ev.targetDead && ev.kill) lines.push(...describeKill(ev.kill, ev.targetName)); break; }
      default: break;
    }
  }
  return lines;
...
export function describeBattleEnd(result) {
  ...
  if (result.cleared) {
    const lines = ['戦闘に勝利した！']; lines.push(`経験値${result.expGained}・ゴールド${result.goldGained}を獲得！`);
    for (const itemId of result.items || []) lines.push(`${itemDisplayName(itemId)} を手に入れた！`);
    if (result.firstClear) lines.push('初回クリア報酬を獲得した！'); return lines;
  }
  return ['力尽きた…戦闘に敗北した。'];
}
