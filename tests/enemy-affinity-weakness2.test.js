...
test('authored elementResist remains authoritative and all values stay capped',()=>{
  const p=enemyAffinityProfile({speciesId:'skeleton',elementResist:{light:-.9,dark:.9}});
  assert.equal(p.resist.light,-.45);
  assert.equal(p.resist.dark,.35);
  assert.equal(affinityMultiplierFromResist(p.resist.light),1.45);
