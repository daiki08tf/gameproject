...
test('recruitment is type-based and elite recruits have a rarity floor',async()=>{const text=await source('js/patches/companionRecruitment.js'),c=compact(text);assert.match(text,/grunt:\s*'goblin'/);assert.match(text,/fast:\s*'bat'/);assert.match(c,/RECRUIT_SPECIES_BY_ENEMY_TYPE\[enemy\.type\]/);assert.match(c,/minRarity:'rare'/);});

test('C6-1: recruitment auto-resolves with no accept/decline prompt -- a successful roll always creates the companion (unless the ranch is full), announced by a toast instead of a modal',async()=>{const text=await source('js/patches/companionRecruitment.js');assert.doesNotMatch(text,/confirm\(|window\.confirm/);assert.match(text,/function resolveRecruitCandidate/);assert.match(text,/ranchHasSpace/);assert.match(text,/reason:\s*'full'/);assert.match(text,/import \{ showToast \} from '\.\/toastFeedback\.js';/);assert.match(text,/仲間になった/);});
