...
test('representative jobs combine both parent identities',()=>{const merc=fusionCombatIdentity('fusion_warrior_merchant');assert.deepEqual(merc.command.effects,['guard','supply']);const ruin=fusionCombatIdentity('fusion_thief_scholar');assert.deepEqual(ruin.command.effects,['crit','analysis']);});
