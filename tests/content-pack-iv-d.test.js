...
test('CP4-4 is observation-only in the existing Stage-list Region UI, scoped per Chapter',()=>{const runtime=fs.readFileSync(new URL('../js/patches/contentPackIVD.js',import.meta.url),'utf8');assert.match(runtime,/state\.data\.world2/);assert.match(runtime,/traversable:false/);assert.match(runtime,/totalBranchCountHidden:true/);assert.match(runtime,/stageList/);
  // M9 replaced the original Ch2-only "第2章" display-text match (journeyName()
  // strips any "第N章" prefix before display, so it could never actually match)
  // with the Chapter number stageSelect.js stamps on #stageSelectScreen — this
  // now scopes each anchor to its own authored chapterNum instead of only Ch2.
  assert.match(runtime,/dataset\.chapterNum/);
  assert.match(runtime,/anchor\.chapterNum===currentChapterNum/);
  assert.match(runtime,/重なりを観測/);assert.doesNotMatch(runtime,/onPick\(|buildSecretRealmStage|teleport|portal|branchTravel|goContentPackIV|contentPackIVScreen/i);});
