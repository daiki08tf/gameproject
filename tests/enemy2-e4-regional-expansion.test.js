...
test('E4 authors five new regional identities for all 41 story chapters',()=>{
  assert.equal(chapterIds.length,41);
  assert.equal(Object.keys(REGIONAL_ENEMY_EXPANSION).length,41);
...
test('E4 registers 205 reusable regional enemy types with stable ecology metadata',()=>{
  let count=0;
  for(const chapterId of chapterIds){
    for(const role of REGIONAL_ENEMY_ROLES){
