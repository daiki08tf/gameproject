...
test('E4 authors five new regional identities for all 33 story chapters',()=>{
  assert.equal(chapterIds.length,33);
  assert.equal(Object.keys(REGIONAL_ENEMY_EXPANSION).length,33);
...
test('E4 registers 165 reusable regional enemy types with stable ecology metadata',()=>{
  let count=0;
  for(const chapterId of chapterIds){
    for(const role of REGIONAL_ENEMY_ROLES){
