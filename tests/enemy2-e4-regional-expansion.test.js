...
test('E4 authors five new regional identities for all 39 story chapters',()=>{
  assert.equal(chapterIds.length,39);
  assert.equal(Object.keys(REGIONAL_ENEMY_EXPANSION).length,39);
...
test('E4 registers 195 reusable regional enemy types with stable ecology metadata',()=>{
  let count=0;
  for(const chapterId of chapterIds){
    for(const role of REGIONAL_ENEMY_ROLES){
