...
test('homeNavigation.js imports settlementCompactUi.js after every settlementContent-writing patch, so its observer registers (and thus applies) last', () => {
  ...
  const writers = [
    ...
    'settlementFortuneTellingUi.js', 'fishingUi.js', 'archaeologyUi.js', 'treasureHuntUi.js', 'settlementRanch3Ui.js',
  ];
...
test('Every data-settlement-*/data-fishing root marker actually rendered by the 14 settlementContent-writing files is covered by SELECTOR_TAB (no silently-unreachable section)', () => {
  const writerFiles = [
    ...
    'settlementSeasonsUi.js', 'settlementIdentityUi.js', 'settlementExpeditionsUi.js', 'settlementEndgameNetworkUi.js',
    'settlementArenaUi.js', 'settlementChronicleUi.js', 'settlementCapitalUi.js', 'settlementUi4.js',
    'settlementFortuneTellingUi.js', 'fishingUi.js', 'archaeologyUi.js', 'treasureHuntUi.js', 'settlementRanch3Ui.js', 'ch1RumorThreadsCampfireUi.js',
  ];
  const markerPattern = /dataset\.(settlement[A-Za-z]+|fishing|archaeology|treasureHunt|ch1Campfire)\s*=\s*['"]true['"]|data-settlement-area=|data-fishing-session-for=/g;
  const found = new Set();
  for (const file of writerFiles) {
    const source = read(`js/patches/${file}`);
    for (const m of source.matchAll(/dataset\.(settlement[A-Za-z]+|fishing|archaeology|treasureHunt|ch1Campfire)\s*=\s*['"]true['"]/g)) found.add(m[1]);
  ...
  for (const camel of found) {
    ...
    if (camel === 'treasureHunt') { assert.match(src, /\[data-treasure-hunt\]/, 'data-treasure-hunt must be in SELECTOR_TAB'); continue; }
