...
test('World 3 branch summary counts discovered and unlocked routes',()=>{
  const summary=world3BranchSummary({
    keyFragments:7,
    keyCount:2,
    secretSites:[{state:'hidden',unlocked:false},{state:'discovered',unlocked:false},{state:'unlocked',unlocked:true}],
    riftKeys:[{id:'a'},{id:'b'}],
  });
  ...
  assert.match(label,/鍵片 7/);
  assert.match(label,/異界 1/);
  assert.match(label,/裂界鍵 1/);
});
