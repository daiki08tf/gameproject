...
test('W33 final UI stays mobile-first and does not add a second Home entry',()=>{
  ...
  assert.doesNotMatch(ui,/appendChild\([^\n]*homeScreen|createElement\('.*home/i);
...
test('W34 Adventure save owns navigation only and does not duplicate canonical progression roots',()=>{
  const session=read('js/patches/adventureWorld4Session.js');
  for(const forbidden of ['characterLevel','worldTierId','bountyNemesis','inventory','equipment','stageProgress','world2.discoveries'])assert.doesNotMatch(session,new RegExp(`${forbidden}\\s*:`));
});
