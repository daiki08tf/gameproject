...
test('World 3 exposes discovered branches while preserving the boundary-key route',()=>{const journey=fs.readFileSync(new URL('../js/screens/chapterSelect.js',import.meta.url),'utf8');const stage=fs.readFileSync(new URL('../js/screens/stageSelect.js',import.meta.url),'utf8');assert.match(journey,/発見された分岐/);assert.match(journey,/world3-branches/);assert.match(stage,/境界鍵路/);assert.match(stage,/world2ForgeKey/);assert.match(stage,/出撃時に鍵を1本消費/);});
