...
## 17. Live viewport pass — completion record (UIX-6 batch 5 cycle — UIX-6 CLOSEOUT)

Same method as §10–§16 (git-archive checkout of the UIX-6 batch 5 commit; QA-only `js/__qaHook.js`, unchanged — Abyss unlocks naturally once `__qaProgress()` clears all ten chapters, no extra grant needed; 390×844/375×667/desktop; console/page-error listeners throughout).

Batch scope resolution: "Abyss / Rift / Secret Realm / Machine Realm / Bounty/Nemesis" names flavor content, not five screens. `js/screens/abyss.js` is the only Abyss screen. Grepping for every id it exposes (`abyssList`/`abyssScreen`/`abyssTreeContent`/`abyssHint`/`abyssShardText`) found no other file writing into it — no MutationObserver-decorator pattern here, unlike every other UIX-6 batch. "Rift" (`rift_scar`) and its sibling named routes (`armory`/`beast_den`/`blood_mist`/`golden_vault`/`veil_fracture`) are entries in `data/abyssRoutes.js` rendered by this same screen's `renderRouteChoices()`. "Secret Realm" is entered the same way via this screen's `renderExploration()` (`data/secretRealms.js` itself carries no `icon` field at all — confirmed by direct grep). "Bounty/Nemesis" is `js/patches/adventureWorld4LivingWorldUi.js`, rendered into the Adventure Route screen (UIX-3 territory) — read in full and confirmed already emoji-free, unrelated to this batch's file. "Machine Realm" was searched for directly (`機械`/`Machine Realm`) and found only as authored Story/canon flavor text (`機械装甲` trait, `未知の機械音` discovery hint) — no implemented screen exists for it at all.

`js/screens/abyss.js` itself carried 10 literal glyphs (🔒🧭🚪🔎⚖🔥☠⚖🔥🔹) confirmed by direct scan, plus 2 more render sites reading category-2 `icon` fields with zero literal glyphs of their own in this file: `route.icon` (the actual source of the `🌀`/`👁️`/`⚔️`/`🐲`/`🩸`/`💰` glyphs every prior batch's live-pass screenshot had shown at this screen, e.g. §11's and §15's "02-abyss" captures) and `c.icon` (from `data/abyssChallenges.js`, e.g. the `🫀`/`☠️` Challenge Modifier icons). This is the same blind-spot pattern every batch since §13 has found — one file, one pass, both literal and consumption sites fixed together this time since the whole batch fit in a single file.

Paths walked: on the fresh save, Abyss's locked state (button class check only — the screen is correctly never opened, matching the established pattern since §10, because it is genuinely locked before all ten chapters clear); on the progressed save (all ten chapters cleared via `__qaProgress`), both Abyss tabs (挑戦/深淵ツリー) at all three viewports, covering Raid, Horizontal Mastery, Exploration, Pacts, Challenges, Route Choices and the Tree panel in one page-text scan per tab per viewport.

Result: zero rendered emoji across all 130 fresh-run and 125 progressed-run captured steps, at every viewport — a clean sweep with no exceptions of any kind (not even an out-of-scope glyph from a later batch, since Abyss was the last screen carrying documented emoji debt). Zero new console/page errors beyond the same pre-existing, already-documented items across §10–§16 (benign `favicon.ico` 404; the UIX-3 `安全に帰還する` residual; an occasional desktop battle-RNG variance already noted in §11's own "informational" framing).

No undocumented runtime-created surface was found. Confirms this ownership map for the Abyss family and this batch's scope-resolution reasoning for Rift/Secret Realm/Machine Realm/Bounty-Nemesis.

**This closes UIX-6.** Every batch (1–5) has a completion record above (§13–§17) confirming its source contract and live-viewport gate. `scripts/uix-emoji-check.js`'s `MAX_APP_PICTOGRAPHS` ceiling has been ratcheted down across all five batches from 364 (UIX-3 baseline) to 266 — 98 glyphs removed from application UI across the whole program, with zero new pictographs introduced at any point. UIX-7 (Motion, Feedback and Accessibility) is next per `UI_OVERHAUL_ROADMAP.md` §10.
