...
- `world2.js` / `world2Core.js` remain authoritative for realm visibility, key fragments, forging and key consumption.
- `exploration1.js` / `exploration1Core.js` remain authoritative for discovery/clue/unlock state.
- `secretRealms.js` remains authoritative for Secret Realm stage construction.
- `riftKeyCore.js` remains authoritative for Rift Key inventory/modifiers.

## Phase 6 next targets
1. Bind Heaven / Underworld realm opening to explicit World nodes instead of status-only visibility.
