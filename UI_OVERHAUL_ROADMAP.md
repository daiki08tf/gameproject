...
UIX-6 is now fully complete, source and live-viewport gate both. UIX-7 (Motion, Feedback and Accessibility Pass, §6 below) is in progress — Phases 1, 2 and 3a are complete; the systemic px→rem dynamic-text retrofit is the one remaining open scope question.
...
- [x] short transition rules — button/bar transitions already existed pre-UIX-7; screen-level fade-in added (Phase 2);
- [x] damage, loot, unlock and discovery feedback — Result-screen loot entrance motion + jackpot/special visual weight, and a shared toast helper covering five previously sound-only success moments (Phase 3a); HP/MP/XP bar-width animation (pre-existing) remains the damage-feedback mechanism;
...
#### [x] Phase 2 — Screen-level transition rules ✅ COMPLETE

Every `.screen` show/hide across the app runs through one of 12 near-identical local `showScreen(id)` implementations, all doing the same `querySelectorAll('.screen').forEach(s=>s.classList.remove('active')); getElementById(id)?.classList.add('active')`. CSS cannot animate across a `display:none` boundary, and rewriting all 12 call sites to a two-step class-toggle (or consolidating them into one shared function) would be real, higher-risk work touching a dozen+ files' navigation logic for a phase whose whole point is to stay narrowly scoped. Found a CSS-only path instead that needed zero JS changes: swap `.screen`'s `display: none ⟷ flex` toggle for `visibility: hidden ⟷ visible` (always `display: flex`), which — unlike `display` — participates in the box tree and so *can* be transitioned/animated, while still correctly removing an inactive screen from hit-testing and the keyboard tab order (unlike `opacity` alone).

Implemented source contract (`css/style.css` only):

- `.screen` (base): `display: none` → `display: flex` (always), plus new `visibility: hidden` and `pointer-events: none`. No `transition` is declared on `visibility` itself, so it flips to hidden the instant `.active` is removed — an immediate cut, not a cross-fade, so the outgoing and incoming screens never visually overlap.
- `.screen.active`: gains `visibility: visible`, `pointer-events: auto` (both instant, applied the moment the class is added — not gated behind the animation), and `animation: dc-screen-in var(--dc-motion-base) var(--dc-ease)` — a short opacity-only fade-in (`--dc-motion-base` is UIX-1's existing 160ms token, not a new hardcoded duration).
- Because `visibility`/`pointer-events` apply immediately and only the visual fade is animated, a repeated action that immediately follows a screen change (e.g. attacking into the next Stage) is never blocked waiting on the animation — satisfies "no animation that delays repeated farming actions" for this new motion too, not just the pre-existing Battle-loop case Phase 1 already confirmed.
- The existing app-wide `@media (prefers-reduced-motion: reduce)` rule in `darkChronicle.css` (from UIX-1) already targets `*, *::before, *::after` with `animation-duration: .01ms !important` — it covers `dc-screen-in` automatically; no new reduced-motion rule was needed.
- `#titleScreen` is marked `class="screen active"` statically in `index.html`, so the very first screen the player ever sees also gets the same brief fade-in on cold load — a deliberate, coherent side effect (the first state change getting the same treatment as every other one), not a bug.
- `tests/uix7-phase2-screen-transitions.test.js` (5 tests) locks the `display:none` removal, the instant (untransitioned) visibility cutover, the immediate `pointer-events`/`visibility` application on `.active`, the motion-token (not hardcoded-duration) usage, and confirms the existing reduced-motion rule already covers the new animation.

Live-viewport pass (390×844/375×667/desktop, fresh-save; progressed-save at 390×844): full existing walkthroughs re-run unchanged (same reasoning as Phase 1 — this touches shared CSS, not one screen, so existing per-screen QA coverage doubles as the regression check). Visually confirmed clean, fully-opaque, non-overlapping screens at Title (including the cold-load fade), Home, Battle, and a nested sub-screen navigation case (Settlement → Market) with no double-exposure or stuck-invisible content. Zero rendered emoji across all 130 fresh-run and 125 progressed-run steps at every viewport; zero new console/page errors beyond the same pre-existing benign items documented since §10. The QA scripts' post-action wait was bumped from 150ms to 220ms (comfortably past the 160ms fade) so screenshots capture the fully-settled state rather than a mid-fade frame — a QA-script-only change, not app code.

#### [x] Phase 3a — Loot/unlock feedback ✅ COMPLETE

Phase 1's audit found exactly one toast in the whole app (Abyss Synergy Unlocked) and otherwise no dedicated visual layer for loot/unlock moments beyond plain text — but the Result screen's *text* reporting (EXP/gold/loot lines, tone-coloring, the existing `equipment3DropHeadline()` "――JACKPOT――"/"――SPECIAL DROP――" strings) was already extensive. The actual gap was specifically in motion and visual weight, not information, and several genuine "you just achieved something" moments (Blacksmith weapon awakening, 極Affix rolls, Rebirth inheritance, Rebirth awakening-rank claims) had a success sound (`Audio_.jobMastered()`) but literally zero visual acknowledgement. Scoped this phase to those two concrete gaps rather than inventing a new rarity-signaling or notification system from scratch, per the Dark Chronicle "avoid excessive glow/particles" restraint.

Implemented source contract:

- `css/style.css`: `.result-item-chip`/`.result-loot-headline` (in `js/screens/result.js`'s `#resultItems`, rebuilt fresh via `innerHTML=''` on every `renderResult()` call — so a pure-CSS entrance animation replays automatically with zero JS retrigger logic needed) get a short `dc-result-in` keyframe (opacity 0→1, `translateY(6px)→0`, using the existing `--dc-motion-base`/`--dc-ease` tokens, not new hardcoded values). `.result-drop-wrap.eq3-jackpot .result-item-chip` and `.eq3-special .result-item-chip` — class names already emitted by `appendDropChip()` from `equipment3Presentation.js`'s `quality` field but never styled before this phase — get a restrained brass border/gradient tint, so the jackpot/special-tier chip reads with more visual weight than a standard drop instead of equal weight.
- `js/patches/toastFeedback.js` (new): `showToast(text, ms=2200)`, a shared helper extracted from `js/main.js`'s one existing inline toast use. Restarts the `.toast` element's existing `toastPop` CSS animation on repeated/back-to-back calls via a forced `display:none` → reflow (`void toast.offsetWidth`) → restore sequence, rather than a bare `classList.remove('hidden')` (which would not restart an animation on an element already visible).
- `js/main.js`: the Abyss Synergy Unlocked toast deduped onto `showToast()`; no behavior change.
- `js/screens/blacksmith.js`: three success moments — weapon awakening (`awakenWeapon`), 極Affix roll (`rollAffix`), second-slot 極Affix roll (`rollAffix2`) — now call `showToast()` alongside their existing `Audio_.jobMastered()`.
- `js/screens/rebirthModern.js`: two success moments — 継承/inheritance (`performInheritance`) and 覚醒Rank claim (`claimAwakeningV2`) — now call `showToast()` alongside their existing `Audio_.jobMastered()`. `js/screens/spellScreen.js`'s own `Audio_.jobMastered()` call site was deliberately left unchanged — it already shows inline text feedback ("ふっかつした！"), so a toast there would be redundant.

Tests: `tests/uix7-phase3a-loot-unlock-feedback.test.js` (7 tests) locks the Result-screen animation and keyframe values, the jackpot/special chip styling, `showToast()`'s display:none-reflow restart mechanism (not a fragile undefined class name), the `main.js` dedup, all five new Blacksmith/Rebirth call sites, and confirms no new emoji were introduced by any of this phase's text. Full suite: 1543/1543 passing. Syntax clean. Emoji gate: unchanged at 265/265 (no literal emoji touched this phase).

Live-viewport pass (390×844/375×667/desktop, fresh-save; progressed-save at 390×844): all four QA runs exit 0, zero rendered emoji, no new console/page errors beyond the same pre-existing benign items documented since §10. Screenshots visually confirmed the Result screen's jackpot-tier chip rendering with the new brass highlight against plain-styled standard chips, and no layout regression on the Blacksmith screen.

Deferred out of this phase, unchanged from Phase 1's finding: the systemic px→rem/`clamp()` dynamic-text-sizing retrofit (dozens of files) — real, separate, much larger work, and arguably already substantially addressed at the leverage level that matters (WCAG 1.4.4) by Phase 1's viewport-zoom fix, which re-enabled the user's own browser-level zoom/text-resize app-wide regardless of any individual class's hardcoded px value. Whether to pursue the retrofit as a further Phase 3b or consider this deliverable adequately closed by Phase 1 + Phase 3a is an open scope decision for the next phase, not assumed here.
...
**UIX-7 is in progress.** Phase 1 (foundation audit + first fixes), Phase 2 (screen-level transition rules) and Phase 3a (loot/unlock feedback) are all complete — see §6 above for the full record of each. Phase 1: the viewport-zoom accessibility fix, the numeric contrast audit, the overlay keyboard/focus helper (`js/patches/overlayA11y.js`) applied to all three ad-hoc overlays in the app, and two more emoji leaks found and fixed along the way (`abyssRunUi.js`, a UIX-6 batch 5 gap; `battleIntegration3.js`/`combat2Elements.js`, a third/fourth site for the `ENEMY_ROLES`/`COMBAT2_ELEMENTS` icon fields already partly fixed in UIX-6 batches 3–4). Phase 2: a CSS-only short fade-in for every screen transition (`.screen` moved from `display:none` to `visibility:hidden`, animated via the existing `--dc-motion-base` token), needing zero changes to the dozen+ files' `showScreen()` implementations. Phase 3a: a short entrance animation plus brass jackpot/special-tier highlighting for Result-screen loot chips, and a new shared `showToast()` helper (`js/patches/toastFeedback.js`) deduping the app's one existing toast use and extending visual feedback to five previously sound-only Blacksmith/Rebirth success moments.

Remaining for UIX-7: the systemic px→rem/`clamp()` dynamic-text-sizing retrofit (dozens of files) that Phase 1's audit named and deliberately deferred — real, separate, much larger work, and arguably already substantially addressed at the WCAG 1.4.4 leverage point by Phase 1's viewport-zoom fix. Whether to pursue this as a further Phase 3b or consider UIX-7's dynamic-text-sizing deliverable adequately closed by Phase 1 + Phase 3a is an open scope decision, not yet made. Follow the same discipline as every prior phase: read the relevant authority files first, keep each phase's diff scoped to one coherent concern, add regression tests, run the full suite/syntax/emoji-gate, and do a live-viewport pass before marking a phase complete. UIX-8 (Real-device Release Readiness) remains after UIX-7 closes.
