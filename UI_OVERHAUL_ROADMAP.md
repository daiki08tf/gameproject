...
UIX-0 through UIX-5 are complete, source and live-viewport gate both. UIX-6 batches 1 (Status / Job / Rebirth), 2 (Companion / Monster Ranch) and 3 (Settlement facilities) are also complete, source and live-viewport gate both. See `UIX0_SOURCE_AND_OWNERSHIP_AUDIT.md` §10–§15 for the full live-viewport record (390×844/375×667/desktop, fresh-save and progressed-save).

The next default task is **UIX-6 batch 4 — Codex / Rumor / records**. Prior audits located this batch's emoji debt at the source level: Monster Codex `🗺⚔`. Batch 5 (Abyss/Rift/Secret Realm/Machine Realm/Bounty-Nemesis, 15 Abyss glyphs) remains a separate PR per this section's own rule. Batch 3 also surfaced one real, deliberately out-of-scope finding for whichever pass owns Battle/Codex presentation: `js/patches/combat3EnemyAI.js` prefixes a researched enemy's Battle-screen name with its `ENEMY_ROLES` icon, a tested (`tests/codex-enemy-knowledge.test.js`) reward-signaling mechanic, not incidental decoration — review it when this batch (Codex) or a Battle-system pass is scoped. Follow the same protocol as UIX-3–6b3: full static source scan for ground-truth emoji counts across every file that renders into the target screen's container, not just the files a prior note named — a prior audit/live pass only proves what it already knew or happened to exercise (batch 3 found 58 glyphs across 17 files against a prior count of 33, including two files whose emoji came from data-object `icon` fields with zero literal glyphs of their own, and one file discovered only by the live pass because its container ids didn't match the initial grep) — focused tests, full `npm test`, `npm run test:syntax`, ratchet `scripts/uix-emoji-check.js`'s ceiling down by the exact count removed, and a live-browser pass at all three required viewports before checking a batch complete.
